...
import { buildMoonConfig, addMoonTask, writeMoonYml } from '../utils/moon.js';
...
async function runScaffold(options: AddOptions) {
  ...
  if (!configResult.success) {
    ...
    if (json) {
      output({ action: 'add', mode: 'scaffold', success: false, error: err }, { json });
    } else {
  ...
  if (json) {
    ...
    outputSuccess(`Scaffolded ${toolName} app '${appName}' at ${join(typeDir, appName!)}`, {
      json,
    });
  }
...
async function runLink(options: AddOptions) {
  ...
  if (!configResult.success) {
    const err = configResult.error?.message ?? 'Could not read project config';
    if (json) {
      output({ action: 'add', mode: 'link', success: false, error: err }, { json });
    } else {
      outputError(err, { json });
    }
    return { success: false };
  ...
  if (json) {
    ...
    outputSuccess(
      `Linked ${toolName} to ${target} (${linkConfig.targetFile})`,
      { json }
    );
  }
...
async function runLocal(options: AddOptions) {
  ...
  if (!discovered) {
    ...
    if (json) {
      output({ action: 'add', mode: 'local', success: false, error: err }, { json });
    } else {
      outputError(err, { json });
    }
    return { success: false };
  ...
  if (json) {
    ...
    outputSuccess(`Forked '${toolName}' to tools/${toolName}`, { json });
  }
...
export async function addCommand(options: AddOptions) {
  const { local, name, target } = options;
  ...
  if (options.json) {
    output({ action: 'add', success: false, error: err }, { json: options.json });
  } else {
    outputError(err, { json: options.json });
  }
  return { success: false };
}
